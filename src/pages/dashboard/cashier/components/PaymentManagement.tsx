...
const PaymentManagement = ({ restaurantId }: PaymentManagementProps) => {
  ...
  const getPaymentMethodText = (method: string) => {
    switch (method) {
      ...
      case 'digital': return 'محفظة رقمية'
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2 space-x-reverse">
          {['today', 'week', 'all'].map((period) => (
            ...
            >
              {period === 'today' ? 'اليوم' : period === 'week' ? 'هذا الأسبوع' : 'الكل'}
            </button>
          ))}
        </div>
      </div>
          ...
          <div className="flex items-center">
            ...
            <div className="mr-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">
                {mockPayments.reduce((total, payment) => total + payment.amount, 0).toFixed(2)} ج.م
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex items-center">
            ...
            <div className="mr-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">
                {mockPayments.filter(p => p.status === 'completed').reduce((total, payment) => total + payment.amount, 0).toFixed(2)} ج.م
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex items-center">
            ...
            <div className="mr-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">
                {mockPayments.filter(p => p.status === 'pending').reduce((total, payment) => total + payment.amount, 0).toFixed(2)} ج.م
              </p>
            </div>
          </div>
        </div>
      ...
      {payments.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">لم يتم العثور على مدفوعات في الفترة المحددة</p>
        </div>
      )}
    </div>
  )
