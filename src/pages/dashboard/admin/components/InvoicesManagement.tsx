...
export default function InvoicesManagement() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (historyError) console.error('خطأ في حفظ تاريخ الدفع:', historyError);
  ...
  const sendPaymentReminder = async (invoice: Invoice) => {
    ...
    console.log(`إرسال تذكير للمورد ${invoice.supplier?.name} بخصوص الفاتورة ${invoice.invoice_number}`);
  ...
  const filteredInvoices = invoices.filter(invoice => {
    ...
    const matchesDate = !dateFilter || invoice.invoice_date.startsWith(dateFilter);
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="mr-3">
              ...
              <p className="text-xl font-bold text-gray-900">{invoices.filter(inv => inv.status === 'paid').length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="mr-3">
              ...
              <p className="text-xl font-bold text-gray-900">{invoices.filter(inv => inv.status === 'pending').length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="mr-3">
              ...
              <p className="text-xl font-bold text-gray-900">{invoices.filter(inv => inv.status === 'partial').length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="mr-3">
              ...
              <p className="text-xl font-bold text-gray-900">{invoices.filter(inv => isOverdue(inv.due_date, inv.status)).length}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">إجمالي المبالغ</h3>
          <div className="text-right">
            <p className="text-2xl font-semibold text-gray-900">
              {mockInvoices.reduce((total, invoice) => total + invoice.total_amount, 0).toLocaleString()} ج.م
            </p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">المبالغ المدفوعة</h3>
          <div className="text-right">
            <p className="text-2xl font-semibold text-gray-900">
              {mockInvoices.reduce((total, invoice) => total + invoice.paid_amount, 0).toLocaleString()} ج.م
            </p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">المبالغ المتبقية</h3>
          <div className="text-right">
            <p className="text-2xl font-semibold text-gray-900">
              {mockInvoices.reduce((total, invoice) => total + (invoice.total_amount - invoice.paid_amount), 0).toLocaleString()} ج.م
            </p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">متأخرة</h3>
          <div className="text-right">
            <p className="text-2xl font-semibold text-gray-900">
              {mockInvoices.filter(i => i.status === 'overdue').reduce((total, invoice) => total + (invoice.total_amount - invoice.paid_amount), 0).toLocaleString()} ج.م
            </p>
          </div>
        </div>
      </div>
                ...
                <tr key={invoice.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    ...
                    {invoice.invoice_file_url && (
                      <i className="ri-attachment-line text-blue-500 mr-1" title="يحتوي على ملف مرفق"></i>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-red-600">
                      {(invoice.total_amount - invoice.paid_amount).toLocaleString()} ج.م
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      isOverdue(invoice.due_date, invoice.status) ? 'bg-red-100 text-red-800' : getStatusColor(invoice.status)
                    }`}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewInvoice(prev => ({ ...prev, supplier_id: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewInvoice(prev => ({ ...prev, subtotal: parseFloat(e.target.value) || 0 }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2"
                    min="0"
                    step="0.01"
                    required
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewInvoice(prev => ({ ...prev, tax_amount: parseFloat(e.target.value) || 0 }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2"
                    min="0"
                    step="0.01"
                  />
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex justify-between text-sm">
                  ...
                  <span>{newInvoice.subtotal.toFixed(2)} ر.س</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span>الضريبة:</span>
                  <span>{newInvoice.tax_amount.toFixed(2)} ر.س</span>
                </div>
                <div className="flex justify-between text-lg font-bold border-t pt-2 mt-2">
                  <span>الإجمالي:</span>
                  <span>{(newInvoice.subtotal + newInvoice.tax_amount).toFixed(2)} ر.س</span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-lg font-semibold text-gray-900">
                  المبلغ الأساسي: {selectedInvoice.subtotal?.toLocaleString()} ج.م
                </p>
                <p className="text-sm text-gray-600">
                  الضريبة: {selectedInvoice.tax_amount?.toLocaleString()} ج.م
                </p>
                <p className="text-xl font-bold text-gray-900 border-t pt-2">
                  الإجمالي: {selectedInvoice.total_amount?.toLocaleString()} ج.م
                </p>
                <p className="text-sm text-green-600">
                  المدفوع: {selectedInvoice.paid_amount?.toLocaleString()} ج.م
                </p>
                <p className="text-sm text-red-600 font-medium">
                  المتبقي: {(selectedInvoice.total_amount - selectedInvoice.paid_amount).toLocaleString()} ج.م
                </p>
              </div>
      ...
      {showPaymentHistoryModal && selectedInvoice && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">تاريخ المدفوعات - {selectedInvoice.invoice_number}</h3>
              <button
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    ...
                    <p className="text-lg font-bold">{selectedInvoice.total_amount.toFixed(2)} ر.س</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">المبلغ المدفوع</p>
                    <p className="text-lg font-bold text-green-600">{selectedInvoice.paid_amount.toFixed(2)} ر.س</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">المبلغ المتبقي</p>
                    <p className="text-lg font-bold text-red-600">{selectedInvoice.remaining_amount.toFixed(2)} ر.س</p>
                  </div>
                </div>
              </div>
      ...
      {showFileUploadModal && selectedInvoice && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">رفع ملف الفاتورة</h3>
              <button
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-sm text-gray-600">فاتورة رقم: {selectedInvoice.invoice_number}</p>
                <p className="text-sm text-gray-600">المورد: {selectedInvoice.supplier?.name}</p>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">اختر ملف الفاتورة</label>
                <input
                  type="file"
                ...
                00 mt-1">الملفات المدعومة: PDF, JPG, PNG</p>
              </div>
