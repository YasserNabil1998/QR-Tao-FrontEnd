...
export default function PurchaseProductsManagement() {
  ...
  const mockPriceHistory: { [key: string]: PriceHistory[] } = {
    '1': [
      { id: '1', product_id: '1', price: 18, purchase_date: '2024-01-01', supplier_name: 'مزارع الخليج', quantity: 50 },
      { id: '2', product_id: '1', price: 19, purchase_date: '2024-01-05', supplier_name: 'مزارع الخليج', quantity: 40 },
      { id: '3', product_id: '1', price: 20, purchase_date: '2024-01-10', supplier_name: 'مزارع الخليج', quantity: 35 },
      { id: '4', product_id: '1', price: 22, purchase_date: '2024-01-15', supplier_name: 'مزارع الخليج', quantity: 25 }
    ],
    '2': [
      { id: '5', product_id: '2', price: 8, purchase_date: '2024-01-01', supplier_name: 'شركة الحبوب المتحدة', quantity: 100 },
      { id: '6', product_id: '2', price: 7.5, purchase_date: '2024-01-08', supplier_name: 'شركة الحبوب المتحدة', quantity: 50 },
      { id: '7', product_id: '2', price: 7, purchase_date: '2024-01-14', supplier_name: 'شركة الحبوب المتحدة', quantity: 50 }
    ]
  };
  ...
  const getPriceChangeIcon = (trend: string) => {
    switch (trend) {
      case 'up': return 'ri-arrow-up-line text-red-500';
      case 'down': return 'ri-arrow-down-line text-green-500';
  ...
  const getTrendBadgeColor = (trend: string) => {
    switch (trend) {
      case 'up': return 'bg-red-100 text-red-800';
  ...
  const getTrendText = (trend: string) => {
    switch (trend) {
      case 'up': return 'ارتفاع';
      case 'down': return 'انخفاض';
  ...
  const filteredProducts = products.filter(product => {
    ...
    if (filter === 'price_up') return matchesSearch && product.price_trend === 'up';
    if (filter === 'price_down') return matchesSearch && product.price_trend === 'down';
    if (filter === 'price_stable') return matchesSearch && product.price_trend === 'stable';
    if (filter === 'high_change') return matchesSearch && Math.abs(product.price_change_percentage) > 10;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-900">قائمة منتجات المشتريات</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="mr-3">
              <p className="text-sm font-medium text-gray-600">أسعار مرتفعة</p>
              <p className="text-xl font-bold text-gray-900">{products.filter(p => p.price_trend === 'up').length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="mr-3">
              <p className="text-sm font-medium text-gray-600">أسعار منخفضة</p>
              <p className="text-xl font-bold text-gray-900">{products.filter(p => p.price_trend === 'down').length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="mr-3">
              <p className="text-sm font-medium text-gray-600">أسعار ثابتة</p>
              <p className="text-xl font-bold text-gray-900">{products.filter(p => p.price_trend === 'stable').length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="mr-3">
              <p className="text-sm font-medium text-gray-600">تغيير كبير</p>
              <p className="text-xl font-bold text-gray-900">{products.filter(p => Math.abs(p.price_change_percentage) > 10).length}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">فلترة حسب الاتجاه</label>
            <select
              value={filter}
            ...
            >
              ...
              <option value="high_change">تغيير كبير (&gt;10%)</option>
            </select>
          </div>
          <div>
                ...
                <tr key={product.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">
                      {product.current_price} ج.م/{product.unit}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-600">
                      {product.previous_price} ج.م/{product.unit}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className={`text-sm font-medium flex items-center ${getPriceChangeColor(product.price_change_percentage)}`}>
                      <i className={`${getPriceChangeIcon(product.price_trend)} ml-1`}></i>
                      {product.price_change_percentage > 0 ? '+' : ''}{product.price_change_percentage.toFixed(2)}%
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getTrendBadgeColor(product.price_trend)}`}>
                      {getTrendText(product.price_trend)}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {product.average_price} ج.م/{product.unit}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {product.total_purchased_quantity} {product.unit}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {new Date(product.last_purchase_date).toLocaleDateString('ar-SA')}
                    </div>
                  </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">تاريخ أسعار المنتج</h3>
              <button
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg">
              <h4 className="text-md font-medium mb-3">تطور السعر عبر الزمن</h4>
                ...
                {priceHistory.map((entry, index) => (
                  <div key={entry.id} className="flex flex-col items-center">
                    <div
                      ...
                      style={{
                        height: `${(entry.price / Math.max(...priceHistory.map(h => h.price))) * 100}px`,
                        width: '30px'
                      }}
                    ></div>
                    <div className="text-xs text-gray-600 mt-1 transform -rotate-45 origin-top-right">
                      {new Date(entry.purchase_date).toLocaleDateString('ar-SA', { month: 'short', day: 'numeric' })}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {priceHistory.map((entry, index) => {
                    ...
                    const priceChange = ((entry.price - previousPrice) / previousPrice) * 100;
                    
                    return (
                      <tr key={entry.id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {new Date(entry.purchase_date).toLocaleDateString('ar-SA')}
                        </td>
