...
export default function ChefSidebar({ activeTab, setActiveTab, isOpen = false, onClose }: ChefSidebarProps) {
  ...
  return (
    <div className={`dashboard-sidebar ${isOpen ? "open" : ""}`}>
      <div
        className={`flex items-center h-14 sm:h-16 px-3 sm:px-4 bg-green-500 border-b border-green-600 ${
      ...
      >
        ...
        >
          <button
            ...
            className="flex items-center space-x-1 sm:space-x-2 space-x-reverse px-2 sm:px-3 py-1.5 text-white hover:text-green-200 transition-colors rounded-lg hover:bg-white/10 border border-white/20 font-medium"
            title={
              direction === "rtl"
                ? "Switch to English"
                : "التبديل إلى العربية"
          ...
          <button
            onClick={onClose}
            className="lg:hidden p-1.5 text-white hover:text-green-200 hover:bg-white/10 rounded-lg transition-colors"
