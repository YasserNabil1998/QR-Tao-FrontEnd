...
const OrderSummary = ({ restaurant, table, cart, totalAmount, onClose, onOrderComplete }: OrderSummaryProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4">
          <div className="mb-6">
            <h3 className="font-medium text-gray-900 mb-2">تفاصيل المطعم</h3>
          ...
          <div className="mb-6">
            <h3 className="font-medium text-gray-900 mb-3">عناصر الطلب</h3>
            <div className="space-y-2">
              {cart.map((item) => (
                <div key={item.cartId} className="flex justify-between items-center py-2 border-b border-gray-100">
                  <div className="flex-1">
                    <p className="font-medium text-sm">{item.name}</p>
                    <span className="text-gray-600">{item.price} ج.م × {item.quantity}</span>
            ...
            <div className="flex flex-col pt-3 border-t-2 border-gray-200 space-y-2">
              <span className="text-lg font-semibold">{subtotal} ج.م</span>
              <span className="text-lg font-semibold">{tax} ج.م</span>
              <span className="text-xl font-bold text-orange-600">{total} ج.م</span>
            </div>
          </div>
